...
from .constants import CALIBRATION_N_BINS, COLORS, DEFAULT_DPI, ECI_COLLAPSE_THRESHOLD
...
def figure1_eci_histograms(
    df: pd.DataFrame, output_path: Path, bins: int = 30, dpi: int = None
):
    """
    ...
    ax.axvline(
        ECI_COLLAPSE_THRESHOLD,
        color=COLORS["red"],
        linestyle="--",
        linewidth=1.5,
        label=f"Collapse threshold ({ECI_COLLAPSE_THRESHOLD})",
    )
...
def figure2_effective_rank_trajectories(
    df: pd.DataFrame, output_path: Path, n_examples: int = 50, dpi: int = None
):
    """
    ...
    sample_collapsed = df_collapsed.sample(
        n=min(n_examples, len(df_collapsed)), random_state=42
    )
    ...
    ax.set_title(
        f"Collapsed sequences (ECI < {ECI_COLLAPSE_THRESHOLD})", fontweight="bold"
    )
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ...
    ax.set_xlabel("Token window index")
    ax.set_title(
        f"Normal sequences (ECI ≥ {ECI_COLLAPSE_THRESHOLD})", fontweight="bold"
    )
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def figure3_failure_prediction_panel(
    df: pd.DataFrame, output_path: Path, dpi: int = None
):
    """
    ...
    y_prob = 1 / (
        1 + np.exp(-(-df["eci_residualized"].values * 10))
    )  # Sigmoid transform for calibration
    ...
    ax1.plot(
        ...
        color=COLORS["blue"],
        linewidth=2,
        label=f"ROC-AUC = {roc_auc:.3f} [{roc_lower:.3f}, {roc_upper:.3f}]",
    )
    ax1.plot(
        [0, 1],
        [0, 1],
    ...
    ax2.plot(
        ...
        color=COLORS["orange"],
        linewidth=2,
        label=f"PR-AUC = {pr_auc:.3f} [{pr_lower:.3f}, {pr_upper:.3f}]",
    )
    ax2.axhline(
        baseline,
        color=COLORS["gray"],
...
def generate_all_figures(
    run_dir: Path, output_dir: Path, smoke: bool = False, dpi: int = None
):
    """
    ...
    """
    from .utils import (
        load_metrics_external,
        load_metrics_internal,
        merge_metrics,
        subsample_data,
    )
    ...
    figure2_effective_rank_trajectories(
        df, output_dir / f"fig2_effective_rank_trajectories{suffix}.png", dpi=dpi
    )

    print("\nGenerating Figure 3: Failure prediction panel...")
    figure3_failure_prediction_panel(
        df, output_dir / f"fig3_failure_prediction_panel{suffix}.png", dpi=dpi
    )
